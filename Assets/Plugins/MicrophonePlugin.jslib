  ...
  Init: function() {
	...
	if (navigator.getUserMedia) {
	  ...
	  StartRecording: function() {
	    ...
	    if (document.mediaRecorder != undefined) {
	      document.mediaRecorder.start();
		  console.log(document.mediaRecorder.state);
	    }
	  },
	  ...
	  End: function() {
	    ...
	    if (document.mediaRecorder != undefined) {
	      ...
	      console.log(document.mediaRecorder.state);
	      document.mediaRecorder.ondataavailable = function(event) {
	        var blob = new Blob([event.data], { type: 'audio/wav' });
	        var reader = new FileReader();
	        reader.onloadend = function() {
	          var buffer = new Uint8Array(reader.result);
	          // Convert the audio file to a base64 string
	          var voiceText = btoa(String.fromCharCode.apply(null, buffer));
	          // Call the Unity method to pass the voiceText to C#
	          SendMessage('DisplayMics', 'SubmitRecord', voiceText);
	        };
	        reader.readAsArrayBuffer(blob);
	      };
	    }
	    if (document.audioContext != undefined) {
